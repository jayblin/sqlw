...
TEST(Statement, can_execute_statements)
{
	...
	ASSERT_EQ(sqlw::status::Code::OK, stmt.status()) << sqlw::status::verbose(stmt.status());
	...
	ASSERT_EQ(sqlw::status::Code::DONE, stmt.status()) << sqlw::status::verbose(stmt.status());
...
TEST(Statement, can_bind_parameters)
{
...

	{
		...
		stmt
			.prepare("SELECT * FROM user u WHERE u.name = ? OR u.id = ? ORDER BY u.id ASC")
			.bind(1, "clovis", sqlw::Type::SQL_TEXT)
			.bind(2, "1", sqlw::Type::SQL_INT);

		ASSERT_EQ(sqlw::status::Code::OK, stmt.status()) << sqlw::status::verbose(stmt.status());

		stmt([&i, &ss] (sqlw::Statement::ExecArgs args)
			{
				i++;
